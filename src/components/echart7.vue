<template>
  <div class="box" v-if="name">
    <div class="item" :class="item.theme" v-for="(item, index) in result"
         :style="{'transform': 'rotate('+ rotate * index +'deg)', '-webkit-transform': 'rotate('+ rotate * index +'deg)', 'height': lineH + 'rem'}">
      ...
      <div class="node" @mouseover.stop="mouseover($event, item)" @mouseout.stop="mouseout($event)"
           @click="load(item.enterpriseId, item.name, item.flag)">
        <div class="inner">
